...
for valvetime in valvetimes:
    ...
    for i in range(Dropnum):  # Main loop
		...
		state_change_conditions={EventName.Tup: 'Open Left 2'},
		output_actions = [('Valve',1)])
	    sma.add_state(
		state_name='Open Left 2',
		state_timer=ValveOpenTime_L,
		...
		state_change_conditions={EventName.Tup: 'Open Right 2'},
		output_actions = [('Valve',2)])
	    sma.add_state(
		state_name='Open Right 2',
		state_timer=ValveOpenTime_R,
